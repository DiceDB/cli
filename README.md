...
<p align="center">
  <img width="100" height="100" src="https://raw.githubusercontent.com/dicedb/diceroll/master/docs/assets/logo.png" />
</p>

<h3 align="center">Interactive Redis: A Cli for Redis with AutoCompletion and Syntax Highlighting.</h3>

<p align="center">
<a href="https://github.com/dicedb/diceroll/actions"><img src="https://github.com/dicedb/diceroll/workflows/Test/badge.svg" alt="Github Action"></a>
<a href="https://badge.fury.io/py/diceroll"><img src="https://badge.fury.io/py/diceroll.svg" alt="PyPI version"></a>
<img src="https://badgen.net/badge/python/3.8%20%7C%203.9%20%7C%203.10%20%7C%203.11" alt="Python version">
<a href="https://pepy.tech/project/diceroll"><img src="https://pepy.tech/badge/diceroll" alt="Download stats"></a>
</p>
...
Diceroll is a terminal client for redis with auto-completion and syntax
highlighting. Diceroll lets you type Redis commands smoothly, and displays results
in a user-friendly format.

Diceroll is an alternative for redis-cli. In most cases, Diceroll behaves exactly
the same as redis-cli. Besides, it is safer to use Diceroll on production servers
than redis-cli: Diceroll will prevent accidentally running dangerous commands,
like `KEYS *` (see
[Redis docs / Latency generated by slow commands](https://redis.io/topics/latency#latency-generated-by-slow-commands)).

## Features

- Advanced code completion. If you run command `KEYS` then run `DEL`, Diceroll
  will auto-complete your command based on `KEYS` result.
- Command validation. Diceroll will validate command while you are typing, and
  highlight errors. E.g. try `CLUSTER MEET IP PORT`, Diceroll will validate IP and
  PORT for you.
- Command highlighting, fully based on redis grammar. Any valid command in
  Diceroll shell is a valid redis command.
...
- Support connection via URL, `diceroll --url redis://example.com:6379/1`.
- Support cluster, Diceroll will auto reissue command for `MOVED` response in
  cluster mode.
- Store server configuration: `diceroll -d prod-redis` (see [dsn](#using-dsn) for
  more).
...
- <kbd>Ctrl</kbd> + <kbd>C</kbd> to cancel the current typed command, this won't
  exit Diceroll, exactly like bash behaviour. Use <kbd>Ctrl</kbd> + <kbd>D</kbd>
  to send a EOF to exit Diceroll.
...
- Written in pure Python, but Diceroll was packaged into a single binary with
...
- You can change the cli prompt using `--prompt` option or set via `~/.dicerollrc`
  config file.
- Hide password for `AUTH` command.
- Says "Goodbye!" to you when you exit!
- For full features, please see: [diceroll.xbin.io](https://www.diceroll.xbin.io)
...
```
pipx install diceroll
```

### Brew

For Mac users, you can install diceroll via brew 🍻
...
You can also use your Linux package manager to install Diceroll, like `apt` in
Ubuntu (Only available on Ubuntu 21.04+).

```shell
apt install diceroll
```

[![Packaging status](https://repology.org/badge/vertical-allrepos/diceroll.svg)](https://repology.org/project/diceroll/versions)
...
```
wget  https://github.com/dicedb/diceroll/releases/latest/download/diceroll.tar.gz \
 && tar -xzf diceroll.tar.gz \
 && ./diceroll
```

(Check the [release page](https://github.com/dicedb/diceroll/releases) if you
want to download an old version of Diceroll.)

## Usage

Once you install Diceroll, you will know how to use it. Just remember, Diceroll
...
Options:
  ...
  --url TEXT                      Use Redis URL to indicate connection(Can set
                                  with env `DICEROLL_URL`), Example:     redis:/
  ...
  -d, --dsn TEXT                  Use DSN configured into the [alias_dsn]
                                  section of dicerollrc file. (Can set with env
                                  `DICEROLL_DSN`)

  --newbie / --no-newbie          Show command hints and useful helps.
  --dicerollrc TEXT                 Config file for diceroll, default is
                                  ~/.dicerollrc.
...
Diceroll support storing server configuration in config file. Here is a DSN
config:
...
Put this in your `dicerollrc` then connect via `diceroll -d staging` or
`diceroll -d dev`.
...
option or change
[dicerollrc](https://github.com/dicedb/diceroll/blob/master/diceroll/data/dicerollrc)
...
`diceroll --prompt '{host:.5s}'`.

### Configuration

Diceroll supports config files. Command-line options will always take precedence
over config. Configuration resolution from highest to lowest precedence is:

- _Options from command line_
- `$PWD/.dicerollrc`
- `~/.dicerollrc` (this path can be changed with `diceroll --dicerollrc $YOUR_PATH`)
- `/etc/dicerollrc`
- default config in Diceroll package.

You can copy the _self-explained_ default config here:

https://raw.githubusercontent.com/dicedb/diceroll/master/diceroll/data/dicerollrc

And then make your own changes.

(If you are using an old versions of Diceroll, please use the config file below,
and change the version in URL):

https://raw.githubusercontent.com/dicedb/diceroll/v1.0.4/diceroll/data/dicerollrc

### Keys

Diceroll support unix/readline-style REPL keyboard shortcuts, which means keys
...
Diceroll is built and released by `GitHub Actions`. Whenever a tag is pushed to
...
Diceroll favors [poetry](https://github.com/sdispater/poetry) as package
...
You can `tail -f ~/.diceroll.log` to see logs, the log is pretty clear, you can
see what actually happens from log files.

### Catch Up with Latest Redis-doc

Diceroll use a git submodule to track current-up-to-date redis-doc version. To
catch up with latest:

1. Git pull in redis-doc
2. Copy doc files to `/data`: `cp -r redis-doc/commands* diceroll/data`
3. Prettier
   markdown`prettier --prose-wrap always diceroll/data/commands/*.md --write`
4. Check the diff, update Diceroll' code if needed.

## Related Projects

- [redis-tui](https://github.com/mylxsw/redis-tui)

If you like diceroll, you may also like other cli tools by
...
- [diceroll](https://github.com/dicedb/diceroll/) - Client for Redis with
  AutoCompletion and Syntax Highlighting

Diceroll is build on the top of
