...
from .lexer import DicerollLexer
...
def greetings():
    diceroll_version = f"diceroll  {__version__} (Python {platform.python_version()})"
    ...
    home_page = "Home:   https://diceroll.xbin.io/"
    issues = "Issues: https://github.com/dicedb/diceroll/issues"
    display = "\n".join([diceroll_version, server_version, home_page, issues])
    if config.raw:
...
DSN_HELP = """
Use DSN configured into the [alias_dsn] section of dicerollrc file. \
(Can set with env `DICEROLL_DSN`)
"""
URL_HELP = """
Use Redis URL to indicate connection(Can set with env `DICEROLL_URL`), Example:
...
@click.option("--url", default=None, envvar="DICEROLL_URL", help=URL_HELP)
@click.option("-d", "--dsn", default=None, envvar="DICEROLL_DSN", help=DSN_HELP)
@click.option(
    "--newbie/--no-newbie",
    default=None,
    is_flag=True,
...
@click.option(
    "--dicerollrc",
    default="~/.dicerollrc",
    help="Config file for diceroll, default is ~/.dicerollrc.",
)
...
def gather_args(
    ...
    """
    Diceroll: Interactive Redis

    When no command is given, Diceroll starts in interactive mode.

    \b
    Examples:
      - diceroll
      - diceroll -d dsn
      - diceroll -h 127.0.0.1 -p 6379
      - diceroll -h 127.0.0.1 -p 6379 -a <password>
      - diceroll --url redis://localhost:7890/3

    Type "help" in interactive mode for information on available commands
    and settings.
    """
    load_config_files(dicerollrc)
    setup_log()
    logger.info(
        f"[commandline args] host={h}, port={p}, db={n}, user={username},"
        f" newbie={newbie}, dicerollrc={dicerollrc}, decode={decode}, raw={raw}, cmd={cmd},"
        f" rainbow={rainbow}."
    )
    # raw config
    if raw is not None:
