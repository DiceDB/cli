...
from dice.completers import MostRecentlyUsedFirstWordCompleter
from dice.completers import diceCompleter, TimestampCompleter, IntegerTypeCompleter
...
def test_newbie_mode_complete_without_meta_dict():
    ...
    completer = diceCompleter(hint=False)
...
def test_newbie_mode_complete_with_meta_dict():
    fake_document = MagicMock()
    fake_document.text_before_cursor = fake_document.text = "GEOR"
    completer = diceCompleter(hint=True)
    completions = list(completer.get_completions(fake_document, None))
...
def test_dice_completer_update_for_response():
    c = diceCompleter()
...
def test_dice_completer_no_exception_for_none_response():
    c = diceCompleter()
...
def test_completion_casing():
    c = diceCompleter(completion_casing="auto")
    fake_document = MagicMock()
    ...
    c = diceCompleter(completion_casing="upper")
    fake_document.text = fake_document.text_before_cursor = "get"
    assert [
        completion.text for completion in c.get_completions(fake_document, None)
    ] == ["GET", "GETEX", "GETSET", "GETDEL", "GETBIT", "GETRANGE"]

    c = diceCompleter(completion_casing="lower")
    fake_document.text = fake_document.text_before_cursor = "GET"
    assert [
        completion.text for completion in c.get_completions(fake_document, None)
    ] == ["get", "getex", "getset", "getdel", "getbit", "getrange"]
