...
from diceroll.config import config, load_config_files
from diceroll.entry import Rainbow, prompt_message
from diceroll.exceptions import NotSupport
...
def test_client_not_support_hello_command(diceroll_client):
    with pytest.raises(NotSupport):
        diceroll_client.pre_hook("hello 3", "hello", "3", None)
...
def test_prompt_message(diceroll_client, config):
    config.rainbow = False
    assert prompt_message(diceroll_client) == "127.0.0.1:6379[15]> "

    config.rainbow = True
    assert prompt_message(diceroll_client)[:3] == [
        ("#cc2244", "1"),
        ("#bb4444", "2"),
        ("#996644", "7"),
    ]
...
def test_on_connection_error_retry(diceroll_client, config):
    ...
    original_connection = diceroll_client.connection
    diceroll_client.connection = mock_connection
    value = diceroll_client.execute("None", "GET", ["foo"])
    assert value == "hello"  # be rendered

    mock_connection.disconnect.assert_called_once()
    mock_connection.connect.assert_called_once()

    diceroll_client.connection = original_connection


def test_on_connection_error_retry_without_retrytimes(diceroll_client, config):
    ...
    with pytest.raises(redis.exceptions.ConnectionError):
        diceroll_client.execute("None", "GET", ["foo"])
...
def test_not_retry_on_authentication_error(diceroll_client, config):
...
def test_auto_select_db_and_auth_for_reconnect_only_6(diceroll_client, config):
    config.retry_times = 2
    config.raw = True
    next(diceroll_client.send_command("select 2"))
    assert diceroll_client.connection.db == 2

    resp = next(diceroll_client.send_command("auth 123"))
    ...
    assert diceroll_client.connection.password is None

    next(diceroll_client.send_command("config set requirepass 'abc'"))
    next(diceroll_client.send_command("auth abc"))
    assert diceroll_client.connection.password == "abc"
    assert (
        diceroll_client.execute("ACL SETUSER", "default", "on", "nopass", "~*", "+@all")
        == b"OK"
    )


@pytest.mark.skipif("version_parse(os.environ['REDIS_VERSION']) > version_parse('5')")
def test_auto_select_db_and_auth_for_reconnect_only_5(diceroll_client, config):
    config.retry_times = 2
    config.raw = True
    next(diceroll_client.send_command("select 2"))
    assert diceroll_client.connection.db == 2
    ...
    next(diceroll_client.send_command("auth abc"))
    assert diceroll_client.connection.password == "abc"
    next(diceroll_client.send_command("config set requirepass ''"))


def test_split_shell_command(diceroll_client, completer):
    assert diceroll_client.split_command_and_pipeline(" get json | rg . ", completer) == (
        " get json ",
        "rg . ",
    )

    assert diceroll_client.split_command_and_pipeline(
        """ get "json | \\" hello" | rg . """, completer
    ) == (""" get "json | \\" hello" """, "rg . ")


def test_running_with_pipeline(clean_redis, diceroll_client, capfd, completer):
    config.shell = True
    clean_redis.set("foo", "hello \n world")
    with pytest.raises(StopIteration):
        next(diceroll_client.send_command("get foo | grep w", completer))
    out, err = capfd.readouterr()
    assert out == " world\n"


def test_running_with_multiple_pipeline(clean_redis, diceroll_client, capfd, completer):
    config.shell = True
    clean_redis.set("foo", "hello world\nhello diceroll")
    with pytest.raises(StopIteration):
        next(
            diceroll_client.send_command("get foo | grep hello | grep diceroll", completer)
        )
    out, err = capfd.readouterr()
    assert out == "hello diceroll\n"
...
def test_peek_key_not_exist(diceroll_client, clean_redis, config):
    config.raw = False
    peek_result = list(diceroll_client.do_peek("non-exist-key"))
    assert peek_result == ["non-exist-key doesn't exist."]


def test_diceroll_with_username():
...
def test_peek_string(diceroll_client, clean_redis):
    clean_redis.set("foo", "bar")
    peek_result = list(diceroll_client.do_peek("foo"))
...
def test_peek_list_fetch_all(diceroll_client, clean_redis):
    clean_redis.lpush("mylist", *[f"hello-{index}" for index in range(5)])
    peek_result = list(diceroll_client.do_peek("mylist"))
...
def test_peek_list_fetch_part(diceroll_client, clean_redis):
    clean_redis.lpush("mylist", *[f"hello-{index}" for index in range(40)])
    peek_result = list(diceroll_client.do_peek("mylist"))
    assert len(peek_result[0]) == 91


def test_peek_set_fetch_all(diceroll_client, clean_redis):
    clean_redis.sadd("myset", *[f"hello-{index}" for index in range(5)])
    peek_result = list(diceroll_client.do_peek("myset"))
    assert len(peek_result[0]) == 27


def test_peek_set_fetch_part(diceroll_client, clean_redis):
...
def test_peek_zset_fetch_all(diceroll_client, clean_redis):
    clean_redis.zadd(
        "myzset", dict(zip([f"hello-{index}" for index in range(3)], range(3)))
    )
    peek_result = list(diceroll_client.do_peek("myzset"))
...
def test_peek_zset_fetch_part(diceroll_client, clean_redis):
    clean_redis.zadd(
...
def test_peek_hash_fetch_all(diceroll_client, clean_redis):
    for key, value in zip(
        [f"hello-{index}" for index in range(3)], [f"hi-{index}" for index in range(3)]
    ):
        clean_redis.hset("myhash", key, value)
    peek_result = list(diceroll_client.do_peek("myhash"))
    assert len(peek_result[0]) == 28


def test_peek_hash_fetch_part(diceroll_client, clean_redis):
    for key, value in zip(
...
def test_peek_stream(diceroll_client, clean_redis):
    clean_redis.xadd("mystream", {"foo": "bar", "hello": "world"})
    peek_result = list(diceroll_client.do_peek("mystream"))
...
def test_mem_not_called_before_redis_4(config, diceroll_client, clean_redis):
    ...
    diceroll_client.execute = wrapper(diceroll_client.execute)
    clean_redis.set("foo", "bar")
    result = list(diceroll_client.do_peek("foo"))
    assert result[0][1] == ("", "string (embstr), ttl: -1")


def test_mem_not_called_when_cant_get_server_version(
    config, diceroll_client, clean_redis
):
    config.version = None
...
def test_reissue_command_on_redis_cluster(diceroll_client, clean_redis):
    mock_response = diceroll_client.connection = MagicMock()
    mock_response.read_response.side_effect = redis.exceptions.ResponseError(
        "MOVED 12182 127.0.0.1:7002"
    )
    diceroll_client.reissue_with_redirect = MagicMock()
    diceroll_client.execute("set", "foo", "bar")
    assert diceroll_client.reissue_with_redirect.call_args == (
        (
            "MOVED 12182 127.0.0.1:7002",
            "set",
            "foo",
...
def test_reissue_command_on_redis_cluster_with_password_in_dsn(
    ...
    config_obj = load_config_files("/tmp/dicerollrc")
    config.alias_dsn = config_obj["alias_dsn"]

    mock_execute_by_connection = diceroll_client.execute_by_connection = MagicMock()
    with patch("redis.connection.Connection.connect"):
        diceroll_client.reissue_with_redirect(
...
def test_version_parse_for_auth(diceroll_client):
    """
    fix: https://github.com/dicedb/diceroll/issues/418
    """
    diceroll_client.auth_compat("6.1.0")
    assert command2syntax["AUTH"] == "command_usernamex_password"
    diceroll_client.auth_compat("5.0")
    assert command2syntax["AUTH"] == "command_password"
    diceroll_client.auth_compat("5.0.14.1")
    assert command2syntax["AUTH"] == "command_password"
...
def test_version_path(info, version):
    with patch("diceroll.client.config") as mock_config:
        mock_config.no_info = True
        mock_config.pager = "less"
        mock_config.version = "5.0.0"
        mock_config.decode = "utf-8"
        with patch("diceroll.client.Client.execute") as mock_execute:
